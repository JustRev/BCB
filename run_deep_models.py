...
for train, test in kfold.split(X, Y):
    ...
    prec_arr, rec_arr, f1_arr, _ = precision_recall_fscore_support(Y[test], y_pred, average=None, labels=[0,1])
    ...
    prec_neg.append(prec_arr[0])
    prec_pos.append(prec_arr[1])
    prec_all.append(prec_all_fold)
    rec_neg.append(rec_arr[0])
    rec_pos.append(rec_arr[1])
    rec_all.append(rec_all_fold)
    f1_neg.append(f1_arr[0])
    f1_pos.append(f1_arr[1])
...
pos_true_lis = [tok for tok in y_true_all if tok == 1]   
neg_true_lis = [tok for tok in y_true_all if tok == 0]   
